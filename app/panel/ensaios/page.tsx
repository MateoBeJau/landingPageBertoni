...
import { BookOpen, Plus, Pencil, Trash2, Upload, X } from "lucide-react";
...
export default function PanelEnsaiosPage() {
  const { loading: ctxLoading, showToast, handleUpload } = usePanelContext();
  const [items, setItems] = useState<Ensaio[]>([]);
  ...
  async function loadItems() {
    const res = await fetch("/api/panel/ensaios");
    if (res.ok) {
      const data = await res.json();
      setItems(data.ensaios ?? []);
    }
    setLoading(false);
  }
  ...
  function resetForm() {
    setForm({ title: "", slug: "", subtitle: "", description: "", cover: "", priceInfo: "Sob consulta", order: items.length });
  }
  ...
  async function handleUpdate() {
    if (!editingId) return;
    try {
      const res = await fetch(`/api/panel/ensaios/${editingId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleDelete(id: string) {
    ...
    try {
      const res = await fetch(`/api/panel/ensaios/${id}`, { method: "DELETE" });
    ...
    <div className="px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-stone-900">Ensaios</h1>
            <p className="text-sm text-stone-500">{items.length} ensaios</p>
          </div>
          <button
            onClick={openNew}
        ...
        <div className="rounded-xl border border-stone-200 bg-white shadow-sm">
          {items.length === 0 ? (
            <div className="py-16 text-center text-stone-500">
              ...
              <p className="text-sm font-medium">Nenhum ensaio ainda</p>
            </div>
          ) : (
              ...
              {items.map((item) => (
                <div key={item.id} className="flex items-center gap-4 px-5 py-4 transition-colors hover:bg-stone-50">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-stone-400">{item.priceInfo}</p>
                  </div>
          ...
          <div className="fixed inset-y-0 right-0 z-50 flex w-full max-w-lg flex-col bg-white shadow-2xl">
            ...
            <div className="flex-1 space-y-4 overflow-y-auto p-5">
              ...
              <div>
                ...
                <input value={form.priceInfo} onChange={(e) => setForm({ ...form, priceInfo: e.target.value })} className={inputClass} />
              </div>
              <div>
            ...
            <div className="flex gap-3 border-t border-stone-200 bg-stone-50 p-5">
              <button onClick={isEditing ? handleUpdate : handleCreate} className="flex-1 rounded-lg bg-stone-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-stone-800">
                {isEditing ? "Salvar alterações" : "Criar ensaio"}
              </button>
